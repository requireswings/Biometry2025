...
B.  (2 Points) If new members of a species immigrate into a defined patch of habitat at a constant rate of 0.5 per day, what is the expected wait time until the next new arrival?
    ...
    \$ \text{PDF of an exponential distribution}=\lambda e\^{-\lambda x}, \text{ if } x \geq 0 \$
...
E.  (1 Point) We are studying a forest where tree heights are normally distributed with a mean height of 10 meters and a variance of 4 meters. What percent of trees do we expect to fall within a height range of 6-14 meters?
    ...
    95% of samples in a normal distribution are within +- 1 standard deviation from the mean. Therefore, 95% of trees would be between 6 and 14 meters (10 +- 4 meters).
    :::
...
The lognormal Q-Q plot of the penguin mass data is not a straight line (curves away from the trendline), which supports that the penguin mass data is not normally distributed.
:::
