...
Let's assume that viral particles are produced at a constant rate inside the host cell between the time of infection and the time of lysis. If this is true, we would expect, given a constant time of lysis, that the number of bacteriophage particles released at lysis should follow a Poisson distribution. We want to obtain a maximum likelihood estimate for the rate variable $\lambda$ in the Poisson distribution describing the number of bacteriophage particles released per infection.
